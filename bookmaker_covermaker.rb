...
cover_minwidth = 600  # pixels
cover_minheight = 800 # pixels
...
def checkSubmittedCoverSize(final_cover, minwidth, minheight, logkey='')
  coversize_check = true
  if File.file?(final_cover)
    coverwidth = `identify -format "%w" "#{final_cover}"`
    coverwidth = coverwidth.to_f
    coverheight = `identify -format "%h" "#{final_cover}"`
    coverheight = coverheight.to_f
    if coverheight < minheight || coverwidth < minwidth
      # this doubles as user/error message
      coversize_check = "Your submitted cover-file is too small to be used for an egalley cover.\n\n"\
        "Bookmaker requires that cover images be at least #{minwidth} x #{minheight} pixels; "\
          "your submitted cover image is #{coverwidth.to_i} x #{coverheight.to_i} pixels.\n"\
        "An autogenerated cover has been used for this egalley instead.\n\n"\
        "To use a custom cover image, please resubmit your manuscript with accompanying cover-image file of at least #{minwidth} x #{minheight} pixels."
    end
  end
  return coversize_check
rescue => logstring
  return "error occurred while verifying size of submitted cover-file."
ensure
  Mcmlln::Tools.logtoJson(@log_hash, logkey, logstring)
end
...
def watermarkCover(watermark, watermarktmp, final_cover, logkey='')
  ...
  coverwidth = `identify -format "%w" "#{final_cover}"`
  coverwidth = coverwidth.to_f
  wmark_width = `identify -format "%w" "#{watermarktmp}"`
  wmark_width = wmark_width.to_f
  # so if coverwidth is < ideal, we crop @ wmark_ideal_width and resize watermark down
  if coverwidth < wmark_ideal_width
    shave = (wmark_width - wmark_ideal_width) / 2
    `convert "#{watermarktmp}" -shave #{shave}x0 -quality 100 "#{watermarktmp}"`
    `convert "#{watermarktmp}" -resize #{coverwidth} "#{watermarktmp}"`
  else # if file too large we just crop at file width. Good up to 4000px wide
    shave = (wmark_width - coverwidth) / 2
    `convert "#{watermarktmp}" -shave #{shave}x0 -quality 100 "#{watermarktmp}"`
  end
...
# make sure any submitted cover is large enough
coversize_check = checkSubmittedCoverSize(final_cover, cover_minwidth, cover_minheight, 'check_submitted_cover_size')
@log_hash['coversize_check'] = coversize_check

# generate cover, watermark existing cover, or skip
if File.file?(final_cover) && coversize_check == true
...
elsif gen == true || coversize_check != true
